...
class LitTexturedRepeatShader : public Shader
{
protected:
	struct vrtx
	{
...
public:
	LitTexturedRepeatShader(float repeat_scale,std::unique_ptr<Transform>& lo) : light_obj(lo),repeat_scale(repeat_scale)
	{
	}

	LitTexturedRepeatShader() = delete;
	...
	glm::vec3 computeFragmentShader(const glm::vec2& pixel, float w0, float w1, float w2) override
	{
		...
		glm::vec3 color = texture->sampleTexturerRepeat(repeat_scale,frag_tex_coord) * (ambient / 3 + diff * diffStrength / 3 + spec *
			specStrength / 3);
		return color;
	}
	...
	{
		std::unique_ptr<LitTexturedRepeatShader> res = std::make_unique<LitTexturedRepeatShader>(repeat_scale,
			this->light_obj);
